...
function runCommand(command, args, input, options = {}) {
  return new Promise((resolve) => {
    ...
    // Decode through a StringDecoder so a multibyte UTF-8 char split across two
    // "data" chunks isn't turned into replacement chars (�) — both the live
    // stream and the accumulated log stay clean. .end() (on close) flushes any
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (!text) return;
      stdout += text;
      if (typeof options.onStdout === "function") { try { options.onStdout(text); } catch {} }
    });
    child.stderr.on("data", (chunk) => {
      const text = errDecoder.write(chunk);
      if (!text) return;
      stderr += text;
      if (typeof options.onStderr === "function") { try { options.onStderr(text); } catch {} }
    });
...
// Throwaway empty working dir for isolated runs. Created in the OS temp area —
// deliberately NOT under ROOT (which lives in C:\AI): keeping it outside the
// project tree stops Claude Code from walking up into C:\AI\CLAUDE.md (~775
// wasted tokens/call of project memory the debate agents never need) and still
// gives the agent nothing nearby to scan. Unique per call → no races when
// participants run in parallel. The caller removes it in a finally (auto-clean).
function makeSandboxDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "council-room-v2-"));
}
...
async function runCodex(prompt, { workdir, model = "", effort = "auto", outFile = "", logFile = "", onStream, isolated = true, signal, onChild, accountEnv, ephemeral = true } = {}) {
  const codexJs = getCodexJs();
  // When isolated: point --cd at a throwaway temp dir so the agent has nothing
  // nearby to scan and stays outside C:\AI. When opened: --cd is the real workdir.
  let cdTarget = workdir || ROOT;
  let sandboxDir = null;
  if (isolated) cdTarget = sandboxDir = makeSandboxDir();
  try {
    // ephemeral=false persists a rollout-*.jsonl session — the only place Codex
    // records the rate_limits we read for the token monitor (see DATA_SOURCES §3).
    // The token-refresh ping needs it; normal council rounds stay ephemeral.
    const args = [
...
async function runClaude(prompt, { workdir, model = "", effort = "auto", logFile = "", onStream, isolated = true, signal, onChild, accountEnv } = {}) {
  ...
  // Isolated runs use a throwaway temp dir OUTSIDE C:\AI — otherwise Claude Code
  // walks up from the cwd and loads C:\AI\CLAUDE.md into every call (~775 tokens
  // of project memory the debate agents don't need). See makeSandboxDir.
  let cwd = workdir;
  let sandboxDir = null;
  if (isolated) cwd = sandboxDir = makeSandboxDir();
  try {
