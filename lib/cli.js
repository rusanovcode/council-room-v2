...
function runCommand(command, args, input, options = {}) {
  return new Promise((resolve) => {
    ...
    try {
      child = useCmd
        ? spawn("cmd.exe", ["/d", "/c", command, ...args], { cwd, windowsHide: true, stdio: ["pipe", "pipe", "pipe"] })
        : spawn(command, args, { cwd, windowsHide: true, stdio: ["pipe", "pipe", "pipe"] });
    } catch (error) {
    ...
    child.on("error", (error) => {
      clearTimeout(timer);
      resolve({ ok: false, code: -1, stdout, stderr: `${stderr}\n${error.message}`.trim(), timedOut, durationMs: Date.now() - startedAt });
    });
    child.on("close", (code) => {
      clearTimeout(timer);
...
async function runCodex(prompt, { workdir, model = "", effort = "auto", outFile = "", logFile = "", onStream, isolated = true } = {}) {
...
async function runClaude(prompt, { workdir, model = "", effort = "auto", logFile = "", onStream, isolated = true } = {}) {
