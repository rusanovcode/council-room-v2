...
function hasExplicitParticipants(settings) {
  return Boolean(Array.isArray(settings && settings.profiles)
    && Array.isArray(settings.participants) && settings.participants.length >= MIN_PARTICIPANTS);
}
...
// { slot, label, mode, verify, chain: [profileObj…] }. `fallbackSlot` and
// `fallbackLabel` cover the legacy two-slot case (codex/claude).
function resolveSlot(r, byId, fallbackSlot, fallbackLabel) {
  return {
    slot: (r && (r.key || r.slot)) || fallbackSlot,
...
function validateParticipants(participants, profilesArr) {
  ...
  for (const p of participants) {
    ...
    seenKeys.add(key);
    const chain = p.profileIds;
    if (!Array.isArray(chain) || !chain.length) return `participant "${key}" needs at least one profile`;
    for (const id of chain) if (!ids.has(id)) return `participant "${key}" references unknown profile "${id}"`;
  }
  return null;
}
