...
async function chatCompletion(prompt, opts = {}) {
  ...
  if (stream) {
    ...
    try {
      for await (const chunk of res.body) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (obj && obj.model) responseModel = String(obj.model || "");
        }
      }
      buf += decoder.end();
    } catch (err) {
    ...
    if (data && data.model) responseModel = String(data.model || "");
  }

  text = String(text).trim();
  const result = { ok: Boolean(text), aborted: false, text: text || `${label}: empty response`, result: { status: res.status, durationMs: Date.now() - startedAt, usage: normalizeUsage(usage), model: responseModel || model } };
...
// OpenRouter account-level failover across a pool of API keys (each a separate
// account = separate free daily quota). Tries keys in the given order; advances to
// the next only on a 429 (rate-limit / daily free cap) — a real error (bad model,
// 400) would fail identically on every key, so we stop early. Streaming is deferred:
// the full text is emitted once on success to keep the retry loop clean. Sets
// result.usedRef = the key that actually served, so the caller bumps the right quota.
async function openrouterPool(prompt, opts = {}) {
  let last = null;
  const blockedRefs = []; // keys that 429'd before we got an answer
  for (const entry of opts.keyPool) {
    ...
    const r = await chatCompletion(prompt, {
      ...
      onStream: undefined, // no partial streaming during failover
    });
    r.result = r.result || {};
    r.result.usedRef = entry.ref;
    if (r.ok) {
      r.result.blockedRefs = blockedRefs;
      if (typeof opts.onStream === "function") { try { opts.onStream(r.text); } catch {} }
      return r;
    }
    last = r;
    if (r.result.status !== 429) break; // non-quota failure repeats on every key
    blockedRefs.push(entry.ref);
  }
  if (last) { last.result = last.result || {}; last.result.blockedRefs = blockedRefs; }
  return last || { ok: false, aborted: false, text: `${opts.label || "OpenRouter"}: no usable keys in pool`, result: {} };
}

function runProfile(profile, prompt, opts = {}) {
  const cfg = resolveProfile(profile);
  // OpenRouter + a supplied key pool → account-level failover (see openrouterPool).
  if (Array.isArray(opts.keyPool) && opts.keyPool.length
      && (profile.provider === "openrouter" || /openrouter\.ai/i.test(cfg.baseUrl))) {
    return openrouterPool(prompt, {
      ...opts,
      baseUrl: cfg.baseUrl,
