const path = require("node:path");
const { appendJsonl, readJsonl, rewriteJsonl, makeId, now } = require("./store");

const VALID_STATUS = new Set(["open", "pending", "resolved", "frozen"]);
const VALID_MODE = new Set(["LIGHT", "STANDARD", "STRICT", "CRITICAL"]);
const VALID_SCOPE_MODE = new Set(["normal", "proposal"]);
const MAX_TITLE_CHARS = 20000;

function subtasksFile(runDir) {
  return path.join(runDir, "subtasks.jsonl");
}

function loadAll(runDir) {
  return readJsonl(subtasksFile(runDir)).map(normalize);
}

const VALID_BIN = new Set(["", "archive", "trash"]);

function normalize(item) {
  return {
    id: String(item.id || makeId("st")),
    title: String(item.title || "Untitled subtask").slice(0, MAX_TITLE_CHARS),
    status: VALID_STATUS.has(item.status) ? item.status : "pending",
    mode: VALID_MODE.has(item.mode) ? item.mode : "STANDARD",
    scopeMode: VALID_SCOPE_MODE.has(item.scopeMode) ? item.scopeMode : "normal",
    bin: VALID_BIN.has(item.bin) ? item.bin : "", // "" = active stack | "archive" | "trash"
    openedAt: String(item.openedAt || now()),
    resolvedAt: String(item.resolvedAt || ""),
    parentId: String(item.parentId || ""),
    reason: String(item.reason || ""),
    rounds: Number(item.rounds || 0),
    summary: String(item.summary || ""),
  };
}

function activeSubtask(runDir) {
  const all = loadAll(runDir);
  return all.find((item) => item.status === "open" && !item.bin) || null;
}

// Move a subtask to a bin ("archive" | "trash") or back to the stack ("").
function setBin(runDir, id, bin) {
  const target = VALID_BIN.has(bin) ? bin : "";
  const all = loadAll(runDir);
  const item = all.find((s) => s.id === id);
  if (!item) throw new Error(`Subtask ${id} not found`);
  item.bin = target;
  // A binned subtask can't stay "active"; restoring brings it back as pending.
  if (item.status === "open") item.status = "pending";
  rewriteJsonl(subtasksFile(runDir), all);
  return item;
}

function emptyTrash(runDir) {
  const all = loadAll(runDir);
  const kept = all.filter((s) => s.bin !== "trash");
  const removed = all.length - kept.length;
  rewriteJsonl(subtasksFile(runDir), kept);
  return { ok: true, removed };
}

function openSubtask(runDir, { title, mode = "STANDARD", parentId = "", scopeMode = "normal" }) {
  const all = loadAll(runDir);
  for (const item of all) {
    if (item.status === "open") item.status = "pending";
  }
  const subtask = normalize({
    id: makeId("st"),
    title,
    mode,
    scopeMode,
    parentId,
    status: "open",
    openedAt: now(),
  });
  all.push(subtask);
  rewriteJsonl(subtasksFile(runDir), all);
  return subtask;
}

function resolveSubtask(runDir, id, summary = "") {
  return updateStatus(runDir, id, "resolved", { resolvedAt: now(), summary });
}

function freezeSubtask(runDir, id, reason = "") {
  return updateStatus(runDir, id, "frozen", { reason });
}

function reopenSubtask(runDir, id) {
  const all = loadAll(runDir);
  for (const item of all) {
    if (item.status === "open") item.status = "pending";
  }
  const target = all.find((item) => item.id === id);
  if (!target) throw new Error(`Subtask ${id} not found`);
  target.status = "open";
  target.resolvedAt = "";
  rewriteJsonl(subtasksFile(runDir), all);
  return target;
}

function updateStatus(runDir, id, status, extra = {}) {
  const all = loadAll(runDir);
  const target = all.find((item) => item.id === id);
  if (!target) throw new Error(`Subtask ${id} not found`);
  target.status = status;
  for (const [key, value] of Object.entries(extra)) {
    if (value !== undefined) target[key] = value;
  }
  rewriteJsonl(subtasksFile(runDir), all);
  return target;
}

function editSubtask(runDir, id, { title, mode, scopeMode }) {
  const all = loadAll(runDir);
  const target = all.find((item) => item.id === id);
  if (!target) throw new Error(`Subtask ${id} not found`);
  const nextTitle = typeof title === "string" && title.trim() ? title.trim().slice(0, MAX_TITLE_CHARS) : "";
  const nextMode = typeof mode === "string" && VALID_MODE.has(mode) ? mode : "";
  const nextScopeMode = typeof scopeMode === "string" && VALID_SCOPE_MODE.has(scopeMode) ? scopeMode : "";
  if ((target.rounds > 0 || target.status === "resolved") && (nextTitle || nextMode)) {
    throw new Error("Subtask already has rounds or is resolved: only scope mode can be edited");
  }
  if (nextTitle) target.title = nextTitle;
  if (nextMode) target.mode = nextMode;
  if (nextScopeMode) target.scopeMode = nextScopeMode;
  rewriteJsonl(subtasksFile(runDir), all);
  return target;
}

function deleteSubtask(runDir, id) {
  const all = loadAll(runDir);
  const idx = all.findIndex((item) => item.id === id);
  if (idx < 0) throw new Error(`Subtask ${id} not found`);
  if (all[idx].rounds > 0) throw new Error("Subtask already has rounds, cannot delete");
  all.splice(idx, 1);
  rewriteJsonl(subtasksFile(runDir), all);
  return { ok: true, id };
}

function incrementRounds(runDir, id) {
  const all = loadAll(runDir);
  const target = all.find((item) => item.id === id);
  if (!target) return null;
  target.rounds += 1;
  rewriteJsonl(subtasksFile(runDir), all);
  return target;
}

function appendEvent(runDir, event) {
  appendJsonl(path.join(runDir, "subtask-events.jsonl"), { at: now(), ...event });
}

module.exports = {
  loadAll,
  activeSubtask,
  openSubtask,
  editSubtask,
  deleteSubtask,
  resolveSubtask,
  freezeSubtask,
  reopenSubtask,
  incrementRounds,
  setBin,
  emptyTrash,
  appendEvent,
  MAX_TITLE_CHARS,
};
