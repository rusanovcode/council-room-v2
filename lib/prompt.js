...
const QUESTIONS_PROTOCOL = [
  ...
  "  only once ALL participants have marked it resolved; then it is no longer asked.",
...
const STRICT_SCOPE_RULE_PROPOSAL = [
  "=== ⚠️ STRICT SCOPE (COMPLEMENT) ===",
  "Strict scope mode is active:",
  "- ANYTHING not listed in the 'Files in Scope' section of the Knowledge Base is automatically considered OUT of scope.",
  "- It is strictly forbidden to propose changes, actions or decisions that touch files/folders outside 'Files in Scope'.",
  "- 'Files Out of Scope' = the complement of 'Files in Scope' (everything else), even if a specific path is not listed explicitly.",
  "- If 'Files in Scope' is empty on this scope-definition subtask — do not block; propose a minimal safe scope and add it via `KB-patch: files_in_scope: ...`.",
  "",
];
...
function buildDebatePrompt({
  ...
  if (strictScope && domain.guards.scopeApplies) {
    lines.push(...(scopeProposal ? STRICT_SCOPE_RULE_PROPOSAL : STRICT_SCOPE_RULE));
  }
  lines.push(
    `=== STATIC SYSTEM ===`,
    ...domain.systemLines,
...
function parseAgentTail(text) {
  ...
  // Escape any regex metacharacters so anchors/tokens are matched literally —
  // keeps the parser robust if a future profile adds an anchor with special chars.
  const esc = (s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const emptyRe = new RegExp(`^(?:${TAIL_CONTRACT.emptyTokens.map(esc).join("|")})$`, "i");
  const grabList = (raw) => {
  ...
  // Build every matcher from TAIL_CONTRACT (single source of truth).
  const anchorOf = (key) => TAIL_CONTRACT.fields.find((f) => f.key === key).anchor;
  const listFieldMatchers = TAIL_CONTRACT.fields
    .filter((f) => f.list)
    .map((f) => ({ key: f.key, re: new RegExp(`^\\s*${esc(f.anchor)}:\\s*(.*)$`, "i") }));
  const statusRe = new RegExp(`^\\s*${esc(anchorOf("status"))}:\\s*(${TAIL_CONTRACT.statusValues.map(esc).join("|")})\\b`, "i");
  const kbRe = new RegExp(`^\\s*${esc(anchorOf("kbPatch"))}:\\s*(.+)$`, "i");
  const resolvedRe = new RegExp(`^\\s*${esc(TAIL_CONTRACT.signals.resolved)}:\\s*(.+)$`, "i");
  const verifyRe = new RegExp(`^\\s*${esc(TAIL_CONTRACT.signals.verify)}:\\s*(.+)$`, "i");
  const priorityRe = new RegExp(`^\\s*${esc(TAIL_CONTRACT.signals.priority)}:\\s*(.+)$`, "i");
