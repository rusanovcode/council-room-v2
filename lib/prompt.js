...
const STATIC_SYSTEM = [
  ...
  "- У каждого вопроса есть приоритет: critical (блокирует переход к исполнению) или minor",
  "  (второстепенный, можно отложить и догнать позже). По умолчанию — critical.",
  "  Пометить второстепенным: `Priority: Q2=minor`. Повысить обратно: `Priority: Q2=critical`.",
  "  ВАЖНО: повышай minor до critical сразу, как только без него исполнение сломается.",
...
function buildDebatePrompt({
  ...
  const lines = [
    `LANGUAGE: ${language === "en" ? "Write your ENTIRE response in English, even if the task, guidance, and knowledge base are in another language. This overrides the language of the input." : "Пиши весь ответ ПО-РУССКИ, даже если задача, направление и база знаний на другом языке. Это важнее языка ввода."}`,
    "",
  ];
  ...
  if (verify && Array.isArray(verify.batch) && verify.batch.length) {
    ...
    lines.push(`Все КРИТИЧНЫЕ вопросы по подзадаче помечены решёнными. Проверь пакет целиком:`);
    for (const item of verify.batch) {
      lines.push(`  ${item.id}: ${item.text}`);
      if (item.answer) lines.push(`     → ответ: ${item.answer}`);
    }
    if (deferredMinors && deferredMinors.length) {
      lines.push(`ОТЛОЖЕННЫЕ (minor, ещё открыты) — НЕ блокируют, но перепроверь:`);
      for (const q of deferredMinors) lines.push(`  ${q.id} [minor]: ${q.text}`);
      lines.push(`Если какой-то из них на самом деле блокирует исполнение — верни critical: \`Priority: Qn=critical\`.`);
    }
    ...
    lines.push(`Решённые помечай \`Resolved: Q1, Q3\`. Приоритет — \`Priority: Q2=minor\`. Новые — через KB-patch: open_questions:.`, "");
  }
