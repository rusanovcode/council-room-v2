...
function resolveTarget(targetPath, roots) {
  ...
  if (!allowed.some((root) => lowerAbs === root || lowerAbs.startsWith(`${root}${path.sep}`))) {
...
function previewWrite(runDir, id, targetPath, roots) {
  const item = get(runDir, id);
  const abs = resolveTarget(targetPath, roots);
...
function write(runDir, id, targetPath, opts) {
  const preview = previewWrite(runDir, id, targetPath, opts.roots);
