...
const BUILTIN = [
  ...
  {
    id: "free",
    label: { ru: "Без правил", en: "No Rules" },
    guards: { scanApplies: false, scopeApplies: false },
    systemLines: [
      "You are a participant in Council Room v2.",
      "No domain-specific rules, constraints, tone, structure, or limits are imposed by this profile.",
      "Use the active subtask and the user's instructions as the source of truth.",
    ],
    sections: [
...
function parseProfileFile(text, fallbackId) {
  ...
  for (const line of m[1].split("\n")) {
    ...
    if (key === "section") {
      ...
      sections.push({ key: skey, title: parts[1] || skey, tipEn: parts[2] || "", tipRu: parts[2] || "" });
    } else {
      meta[key] = val;
    }
  }
  ...
  if (!sections.some((s) => s.key === "open_questions")) {
    sections.push({ key: "open_questions", title: "Open Questions" });
  }
  return {
    id,
...
function createProfile(input) {
  ...
  if (!sections.some((s) => s.key === "open_questions")) sections.push({ key: "open_questions", title: "Open Questions" });
