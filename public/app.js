...
const STRINGS = {
  ru: {
    ...
    "tip.tokenEconomy": "Почему агенты тратят много токенов и как экономить.\n\nCLI-бэкенды (Codex exec, Claude -p) тащат свой агентный «харнесс» — системный промпт плюс схемы инструментов — ~6000–7000 токенов на КАЖДЫЙ вызов, даже на тривиальный вопрос. Наш промпт дебатов — лишь ~800 токенов из них, поэтому обрезать его почти бесполезно.\n\nРычаги (по убыванию эффекта):\n1) API-бэкенд вместо CLI — харнесса нет, ~850 токенов вместо ~8000 (в ~9 раз дешевле). Дебатам инструменты не нужны: зарегистрируй агента с провайдером Anthropic / Groq / DeepSeek.\n2) Меньше раундов: режим LIGHT плюс авто-закрытие по консенсусу.\n3) Раунды плотно во времени — prompt-кэш живёт минутами.\n4) Изоляция вынесена в системный temp (вне C:\\AI), так что Claude больше не тянет C:\\AI\\CLAUDE.md (~775 токенов/вызов).|||два API-профиля спорщиками (напр. Anthropic claude-haiku-4-5 + Groq) → раунд дебатов ~1700 токенов вместо ~16000 на двух CLI-агентах.",
  ...
  en: {
    ...
    "tip.tokenEconomy": "Why agents burn tokens, and how to save.\n\nCLI backends (Codex exec, Claude -p) carry their own agent harness — system prompt plus tool schemas — ~6,000–7,000 tokens on EVERY call, even a trivial question. Our debate prompt is only ~800 of that, so trimming it barely helps.\n\nLevers (by impact):\n1) API backend instead of CLI — no harness, ~850 tokens vs ~8,000 (~9x cheaper). Debates need no tools: register an agent with the Anthropic / Groq / DeepSeek provider.\n2) Fewer rounds: LIGHT mode plus auto-resolve on consensus.\n3) Keep rounds close in time — the prompt cache lives for minutes.\n4) Isolation now runs in the OS temp dir (outside C:\\AI), so Claude no longer pulls in C:\\AI\\CLAUDE.md (~775 tokens/call).|||two API debater profiles (e.g. Anthropic claude-haiku-4-5 + Groq) → one debate round ~1,700 tokens instead of ~16,000 on two CLI agents.",
