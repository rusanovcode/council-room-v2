...
const STRINGS = {
  ru: {
    ...
    "tip.kb.files_out_of_scope": "Файлы и директории, которые не трогать.|||«node_modules\\*», «.env».",
    ...
    "ui.apiSteps": "Это профиль с API-ключом.|Ключ вводится в поле «API-ключ (прямой ввод)» в панели «Регистрация агентов».|Вход через терминал не нужен.",
    "ui.tabLimits": "Лимиты",
    "ui.tabSpend": "Расход",
    "ui.tabSub": "Подписка",
    ...
    "ui.loginSteps": "Откроется отдельное окно терминала.|В нём запустится команда входа: {cmd}|Следуй подсказкам CLI — обычно откроется браузер.|После сообщения об успехе вернись сюда. Окно терминала можно закрыть.",
    "tip.acctBtn": "{tool} — аккаунт {account}.",
    "tip.switcher": "",
    ...
    "tip.agentBackend": "Какой бэкенд отвечает за этого агента: CLI (Codex/Claude), сетевой API-провайдер или локальная Ollama. Список — из доступного: авторизованные CLI-агенты + зарегистрированные провайдеры.|||«Codex CLI», «Claude CLI», «DeepSeek», «Ollama (локально)».",
  ...
  en: {
    ...
    "tip.kb.files_out_of_scope": "Files and directories that must not be touched.|||'node_modules/*', '.env'.",
    ...
    "ui.apiSteps": "This is an API-key profile.|Enter the key in the «API key (direct entry)» field in the Backends panel.|No terminal login is needed.",
    "ui.tabLimits": "Limits",
    ...
    "ui.loginSteps": "A separate terminal window opens.|It runs the login command: {cmd}|Follow the CLI prompts — a browser usually opens.|After the success message, come back here. You can close the terminal window.",
    "tip.acctBtn": "{tool} — account {account}.",
    "tip.switcher": "",
    ...
    "ui.tokenWarn4": "4 agents: full context ×4, no compression — expensive. That's 4 backend calls per round.",
    ...
    "tip.agentBackend": "Which backend powers this agent: CLI (Codex/Claude), a network API provider, or local Ollama. The list comes from what's available: authorized CLIs + registered providers.|||«Codex CLI», «Claude CLI», «DeepSeek», «Ollama (local)».",
...
const panelOpen = (() => {
  const def = { chatArchive: false, chatTrash: false, subtaskArchive: false, subtaskTrash: false };
