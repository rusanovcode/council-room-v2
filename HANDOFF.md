...
- **`validatedRefs` — in-memory**: при рестарте сервера флаг проверки сбрасывается → ключ, ранее протестированный, показывает ЖЁЛТУЮ галочку (хотя он в `.env`). Решить: персистить validated в файл (как `usage`) ИЛИ авто-перепроверять дешёвым запросом на старте. Сейчас намеренно freshness-only.
- **Чип «готов» завязан на наличие ключа (`credentials`), а не на `validated`** → возможна нестыковка: present-but-unverified ключ = зелёный чип, но жёлтая галочка в поле. Решить, выравнивать ли чип на `validated`.
- **Цвет чипа — только green(ready)/grey(no key)**: бакеты green/yellow/red для API-ключей не применяются (нет источника «остатка %»). Если появится — бакетить в `connectedAgents()`.
- **Визуальная проверка пользователем** в реальном приложении: sticky-панель, сворачивание «Настроек», фикс переполнения, состояния галочки ключа (зел/жёлт), RGB-кнопка — ассистент видел только headless-скриншот api-чипов.
