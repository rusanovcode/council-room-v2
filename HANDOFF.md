...
### Phase 6 — осталось (6b/6c)
- **6b (UX выбора)**: кнопка «Добавить агента» (2 строки) в шапке центра слева; авто/ручной выбор; чипы участников; пересборка выпадашки агента (агент+модель+усилие, без «скорости»); шаги навигатора; **эскалирующее предупреждение о токенах** на каждого добавленного; **тумблер «Файлы вне scope» (`strictScope`) включён по умолчанию** (запрос пользователя). Панель ролей A/B Phase 5 пока работает как есть (через back-compat `roles{a,b}`).
- **6c (реестр)**: «Профили и роли» → «Регистрация агентов» (регистрация + авторизация); новый столбик-иконок зарегистрированных агентов (оверлей справа над чатом, раскрытие влево, не ломая grid). Что отображать в столбике — доисследовать.
- **Живой прогон** реальными агентами (2–5) — за пользователем (подписки). Проверено только моком.
