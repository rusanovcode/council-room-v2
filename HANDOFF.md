...
- **Панель токенов под API-ключи**: у ключей нет «остатка %» → показывать spend или прятать (сейчас серое). Переосмыслить вкладки статистики.
- **Живой прогон** реального API-провайдера / Ollama / CLI-as-provider — за пользователем (ключи/подписки). Проверено только моком.
- **Публичная сборка**: теперь = `PROVIDERS_MODE=api` + положить `.env`. Отдельной вырезки кода НЕ требуется (решили флагом).
...
2. **Спросить пользователя**: что делаем дальше? (Phase 2 закрыт; **Phase 5 ЯДРО закрыто 2026-05-30** — провайдеры/профили/роли, см. §2c. Не закрыто в Phase 5: панель токенов под API-ключи (spend vs остаток %), живой прогон реальных провайдеров. Раньше построены цикл вопросов, мультиаккаунт/свитч+gateway, архив/корзина, статистика — §10. Не закрыто: Phase 3 панель/Phase 4 gate; отложено: Codex OAuth-usage.)
