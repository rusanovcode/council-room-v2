...
- **Isolated mode (NO scan by default)** — критично:
  ...
  - Isolated runs use a throwaway empty dir in the OS temp area (`os.tmpdir()/council-room-v2-*`,
    created per call via `fs.mkdtempSync`, removed in a `finally`), NOT `rooms/_sandbox` under `C:\AI`.
    Codex: `--cd <tempdir>`; Claude: `cwd: <tempdir>` + `--tools ""`.
    Rationale: a cwd outside `C:\AI` stops Claude Code from walking up into `C:\AI\CLAUDE.md`
    (~775 tokens/call of project memory the debate agents don't need). Applies to ALL discussion
    modes when scan is off (`isolated = !allowFilesystemScan`), not just `free`.
...
- ai-switcher (`<repo>/ai-switcher`) — **используется** (модуль свитч). Источники, пути и форматы — в **`DATA_SOURCES.md`**.
