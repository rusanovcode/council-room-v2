...
│   ├── prompt.js             buildDebatePrompt (otherAgentNames[] — [Phase 6] N оппонентов), parseAgentTail, STATIC_SYSTEM, NO_SCAN_GUARD, STRICT_SCOPE_RULE
...
│   ├── profiles.js           [Phase 5/6] схема профиль/участник; effectiveConfig → participants[2..5] (явные settings.participants → явные roles{a,b} → legacy); roles{a,b}=первые 2 (back-compat); legacyChain (acc1/acc2 + failover); CRUD профилей+участников (validateParticipants/setParticipants, MIN=2/MAX=5); VERIFY_AGENTS
...
[Phase 5] Настройки могут содержать `profiles:[{id,label,provider,model,effort,account?,baseUrl?,credentialRef?}]` и `roles:{a,b}` (явная конфигурация спорщиков). Если заданы — переопределяют legacy codex/claude-поля (`profiles.effectiveConfig`). `/api/settings` валидирует `body.profiles`. [Phase 6] Настройки также могут содержать `participants:[{key,label,mode,profileIds[],verify?}]` (2–5 спорщиков) — если заданы, имеют приоритет над `roles{a,b}`; `effectiveConfig` отдаёт `participants[]` (slot=key: legacy `codex`/`claude`, новые `a1..a5`). `/api/settings` валидирует `body.participants` (`validateParticipants`: 2–5, уникальные ключи, непустые цепочки на существующие профили). Вопрос закрывается когда ВСЕ участники его отметили (`questions.recordResolve(...,requiredKeys=partKeys)`). Ключи задаются именем env-переменной (`credentialRef`) — значение в `.env`/окружении, не в `state.json`; ключ можно либо вписать в `.env` руками, либо ввести прямо в UI (поле «API-ключ (прямой ввод)» → `POST /api/providers/key` пишет его в `.env`). `publicState().providers = {mode, presets, types, credentials{profileId→keyPresent}, validated{profileId→keyVerified}, usage{profileId→spend}}` для UI. `PROVIDERS_MODE` (env): `full` (CLI+switcher+OAuth) / `api` (только API-ключи + Ollama, CLI off).
