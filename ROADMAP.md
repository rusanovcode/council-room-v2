...
## Phase 5 — Провайдеры по API-ключу (OpenAI-совместимый адаптер) + настраиваемые «спорщики»

### Цель
Дать публичное/общее развёртывание без подписочных CLI: подключать LLM по **API-ключу**
(включая бесплатные тиры) и **локальные модели** (Ollama). Подписочные `codex`/`claude`
CLI остаются для **личного локального** использования.

### Задачи
- **Универсальный OpenAI-совместимый адаптер** — `run(prompt, {baseURL, apiKey, model})` →
  `{ok, text}` (тот же контракт, что у `runCodex`/`runClaude` в `lib/cli.js`). Покрывает
  DeepSeek, OpenAI API, Groq, OpenRouter, Mistral, Together и пр. (`/v1/chat/completions`).
- **Адаптер Ollama** (`http://localhost:11434`) — бесплатные локальные модели для тестов.
- **Сделать двух «спорщиков» настраиваемыми** — сейчас в раунде жёстко зашиты Codex и
  Claude (`runCodexOn`/`runClaudeOn` в `server.js`). Обобщить выбор «агент A / агент B» →
  `{provider, model, creds}` в настройках чата.
- **Хранение ключей** — env / настройки, НЕ в репозитории (как и auth-файлы).

### Заметки/ограничения (обсудили 2026-05-30)
- «Бесплатно» = бесплатные тиры (всё равно нужен API-ключ) ИЛИ локальные модели; «скачать API» нельзя.
- Подписочные аккаунты (ChatGPT Plus, Copilot) через неофициальные API — нарушение ToS,
  не делаем. Легально: официальный API по ключу / официальный CLI-OAuth / локальные модели.
