...
### Phase 6b — UX выбора (TODO)
- [ ] Кнопка «Добавить агента» (2 строки) в шапке центра слева от Закрыть/Заморозить/Лог; подсветка навигатором.
- [ ] Выбор авто/ручной; авто — 2 разных доступных дешёвых бэкенда; добавление до 5.
- [ ] Чипы участников; клик → пересобранная выпадашка в правом столбце (агент + модель + усилие, **без «скорости»**); дефолт слабая модель+усилие, подсвечены.
- [ ] Навигатор: шаги «добавь агентов» → «выбери модели» → «галочки/документы/запуск»; `▶ Запустить раунд` заблокирован без участников.
- [ ] **Эскалирующее предупреждение о токенах** на каждого добавленного (2 норм … 5 «очень дорого»).
- [ ] **Тумблер «Файлы вне scope» (`strictScope`) включён по умолчанию** (`settings.strictScope=true`); сверить взаимодействие с пустым `files_in_scope`.
- [ ] bilingual + tooltips на все новые элементы.

### Phase 6c — реестр (TODO)
- [ ] «Профили и роли» → «Регистрация агентов» (регистрация + авторизация; редакторы ролей A/B убрать оттуда — выбор переехал в чипы центра).
- [ ] Новый столбик-иконок зарегистрированных агентов: оверлей справа над чатом, раскрытие влево «наплывом» (`position:absolute`, не ломая grid). Содержимое столбика — доисследовать (минимум: иконка + статус авторизации + «добавить в чат / редактировать»).
- [ ] Правка выбранного агента — в существующих «Настройках».
...
| Phase 6 | 6a **CLOSED 2026-05-30** (ядро 2–5 агентов); 6b/6c — средняя (UX выбора + реестр) | высокая — пользовательский запрос на пересборку выбора агентов | 6b/6c поверх 6a |

Рекомендуемый порядок: ~~2~~ → **6a (done) → 6b/6c → 4 → 3**. (Phase 2 закрыт, Phase 6a закрыт.)
