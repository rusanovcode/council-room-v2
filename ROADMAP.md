...
- [x] Проверки: тесты зелёные (+inline-backend юнит-кейс), boot-смоук full (валидация/persist), реальный раунд на 3 inline-бэкендах через мок. Визуальная проверка в браузере — за пользователем.
...
| Phase 6 | 6a+6b **CLOSED 2026-05-30** (ядро 2–5 агентов + UX выбора); 6c — средняя (реестр) | высокая — пользовательский запрос на пересборку выбора агентов | 6c поверх 6a/6b |

Рекомендуемый порядок: ~~2~~ → **6a (done) → 6b (done) → 6c → 4 → 3**. (Phase 2 закрыт, Phase 6a+6b закрыты.)
